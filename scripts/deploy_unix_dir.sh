...
for i in $(ldd ../bin/qbit |awk '{ print $3}' |grep Qt); do cp $i .; done
for i in $(ldd ../bin/qbit |awk '{ print $3}' |grep libic); do cp $i .; done
